...
p_f operator()(type_to_type<uint8_t>) {
    ...
    p_f operator()(type_to_type<type>) {
        return [](uint8_t dst[], const uint8_t* src[], const Size& inSz, const Size& outSz,
                  Q0_16 yalpha, const MapperUnit8U& ymap, int xmaxdf,
                  const short xindex[], const Q0_16 xalpha[], Q8_8 vbuf[]) {
            auto outT = reinterpret_cast<type*>(dst);
            const auto inT = reinterpret_cast<const type**>(src);
#if 1
            auto ya = static_cast<A>(yalpha);
            const auto xa = reinterpret_cast<const A*>(xalpha);
            const auto ym = reinterpret_cast<const MapperUnit<A, I>&>(ymap);
            const auto xidx = reinterpret_cast<const I*>(xindex);
            auto vb = reinterpret_cast<W*>(vbuf);
...
GAPI_FLUID_KERNEL(FScalePlaneArea32f, ScalePlaneArea32f, true) {
    ...
    static void run(const cv::gapi::fluid::View & in, Size /*sz*/, int /*interp*/,
                    cv::gapi::fluid::Buffer & out, cv::gapi::fluid::Buffer & scratch) {
        calcAreaRow<float, float, int, float, areaDownscale32f::Mapper>(in, out, scratch);
    }
};
#endif
GAPI_FLUID_KERNEL(FScalePlaneArea8u, ScalePlaneArea8u, true) {
    ...
    static void run(const cv::gapi::fluid::View & in, Size /*sz*/, int /*interp*/,
        ...
        calcAreaRow<uint8_t, Q0_16, short, Q8_8, areaDownscale8u::Mapper>(in, out, scratch);
    }
};
};
